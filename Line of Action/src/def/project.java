...
class project {
 ...
 return bord;
	...
	public static class Window implements ActionListener {
		...
		public static int Win(Soldier[][] bord,int turn,boolean edge) 
		// 
		{
			int max=0,tot=0,count=0;
		Stack<int[]> wait=new Stack();
		Stack<int[]>chaked=new Stack();
		for(int y=1;y<bord.length-1;y++)
			for(int x=1;x<bord[0].length-1;x++) 
			{
				...
				pice[1]=x;
				 ...
				 {				 
				wait.add(pice);
				int max2=Ser(wait,chaked,turn,bord,y,x,edge);
		...
		public static int Ser(Stack<int[]>wait,Stack<int[]>chaked,int turn, Soldier[][] bord,int y,int x,boolean  edge ) 
		// in pice 0 is the y with the edge
		{  int max=1;
           while(!wait.isEmpty()) 
           {
	...
	{
		 int x=Win(bord,1,true);
		 int y=Win(bord,-1,true);
		 play.SetCon(x, y);
